...
function  find_entry(needle, haystack) {
...
Verifier.prototype = {
  ...
  onPredictPreconnect: function verifier_onPredictPreconnect(uri) {
    ...
    var index = find_entry(origin, this.expected_preconnects);
    if (index == -1) {
  ...
  onPredictDNS: function verifier_onPredictDNS(uri) {
    var origin = extract_origin(uri);
    var index = find_entry(origin, this.expected_preresolves);
    if (index == -1) {
...
function die_if_still_verifying(verifier) {
  if ((verifier.expected_preconnects.length > 0) ||
      (verifier.expected_preresolves.length > 0)) {
    do_check_true(false, "Timed out while testing " + verifier.verifying);
  } else {
...
function test_link_hover() {
  ...
  do_timeout(1000, function () { die_if_still_verifying(verifier); });
}
...
function test_origin() {
  ...
  for (var i = 0; i < subresources.length; i++) {
    ...
    if (find_entry(origin, preconns) === -1) {
